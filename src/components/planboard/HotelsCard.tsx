  ...
  return (
    <div className="mt-3.5 rounded-2xl border border-line bg-white p-[15px]">
      ...
      <div className="grid grid-cols-3 gap-3">
        {hotels.map((h) => (
          <div key={h.name} className="overflow-hidden rounded-[13px] border border-[#eef1f5] bg-[#fbfcfd]">
            <div className="relative h-[74px]" style={{ background: h.gradient }}>
            ...
            <div className="px-[11px] pb-3 pt-2.5">
              <div className="text-[13px] font-bold leading-[1.15] tracking-[-.01em]">{h.name}</div>
              <div className="mt-[3px] text-[11px] text-[#8b98a1]">{h.area}</div>
              <div className="mt-[9px] flex items-baseline justify-between">
                <span className="text-[11px] tracking-[1px] text-gold-2" aria-label={`${h.stars} stars`}>
                  {"★".repeat(h.stars)}
                </span>
                <span className="text-[13px] font-extrabold text-brand-650">
                  {currency}
                  {h.price}
                  <span className="text-[10px] font-semibold text-[#9aa5ac]"> /night</span>
                </span>
              </div>
            </div>
          </div>

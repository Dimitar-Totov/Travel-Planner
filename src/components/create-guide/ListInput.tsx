...
export default function ListInput({
  ...
  return (
    <div>
      ...
      <div className="mt-1.5 flex gap-2 lg:mt-2.5 lg:gap-3">
        <input
          ref={inputRef}
          id={id}
          value={draft}
          ...
          className={`h-11 w-full min-w-0 rounded-[10px] border bg-surface-3 px-3.5 text-[14.5px] text-ink outline-none placeholder:text-[#9fb1bd] focus:bg-white focus:ring-[3px] lg:h-14 lg:px-5 lg:text-[17px] ${
            duplicate
      ...
      {values.length === 0 ? (
        <p className="mt-2.5 text-[12.5px] italic text-muted-2 lg:mt-3.5 lg:text-[15px]">
          {emptyLabel}
        </p>
      ) : variant === "chip" ? (
        <ul className="mt-2.5 flex flex-wrap gap-1.5 lg:mt-3.5 lg:gap-2.5">
          {values.map((value) => (
            <li key={value}>
              <span className="inline-flex items-center gap-1.5 rounded-full bg-brand-500/10 py-1 pl-3 pr-1 text-[12.5px] font-semibold text-brand-700 lg:py-2 lg:pl-4 lg:text-[15px]">
                {value}
                <button
                  type="button"
                  onClick={() => onRemove(value)}
                  aria-label={`Remove ${value}`}
                  className="inline-flex h-[22px] w-[22px] items-center justify-center rounded-full text-brand-700/70 outline-offset-2 outline-brand-500 transition-colors hover:bg-brand-500/20 hover:text-brand-700 focus-visible:outline-2 lg:h-[28px] lg:w-[28px]"
                >
                  <CloseIcon size={12} />
                </button>
              </span>
        ...
        <ul className="mt-2.5 flex flex-col gap-1.5 lg:mt-3.5 lg:gap-2.5">
          {values.map((value) => (
            <li
              key={value}
              className="flex items-start gap-2 rounded-[10px] bg-surface-2 py-2 pl-3 pr-2 text-[13.5px] leading-[1.5] text-muted lg:gap-3 lg:py-3.5 lg:pl-4 lg:text-[16px]"
            >
              ...
              <button
                ...
                onClick={() => onRemove(value)}
                aria-label={`Remove ${value}`}
                className="inline-flex h-[26px] w-[26px] flex-none items-center justify-center rounded-full text-muted-2 outline-offset-2 outline-brand-500 transition-colors hover:bg-danger/10 hover:text-danger focus-visible:outline-2 lg:h-[32px] lg:w-[32px]"
              >
                <TrashIcon size={14} />
              </button>
            </li>
